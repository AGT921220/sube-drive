...
class HomeApiController extends Controller
{
     ...
     */
    public function homeData_bkp(Request $request)
    {
        $homeData = [];
        ...
        $itemTypes = ItemType::where('status', '1')
            ->whereIn('id', function ($query) {
                $query->select('item_type_id')
                    ->from((new Item)->getTable())
                    ->where('status', '1');
            })
            ->get(['id', 'name', 'description', 'status'])
            ->map(function ($itemType) {
                $imageUrl = isset($itemType->image) ? $itemType->image->url : null;
                $items = Item::where('status', '1')
                    ->where('item_type_id', $itemType->id)
                    ->take(10)
                    ->get()
                    ->map(function ($items) {
                        return $this->formatItemData($items);
                    });
        ...
        $homeData['Categories'] = $itemTypes;
        $featuredItems = Item::where('is_featured', 1)
            ->where('status', 1)
            ->orderByDesc('created_at')
            ->take(10)
            ->get()
            ->map(function ($featuredItems) {
                return $this->formatItemData($featuredItems);
            });

        $homeData['Featured'] = $featuredItems;

        return $this->addSuccessResponse(200, trans('global.Result_found'), $homeData);
    }
    ...
    public function nearbyItems(Request $request)
    {
        ...
        try {
            ...
            $nearbyItems = $query->orderBy('id')
                ...
                ->take($limit)
                ->get()
                ->map(function ($item) use ($user, $convertionRate) {
