...
class ItemWishlistController extends Controller
{
    ...
    public function addToWishlist(Request $request)
    {
        ...
        if ($isAlreadyInWishlist) {
            return $this->addErrorResponse(422, trans('global.already_exist'), '');
        }
        ...
        return $this->addSuccessResponse(200, trans('global.Added_successfully'), $item_id);
        try {
        } catch (\Exception $e) {
    ...
    public function removeFromWishlist(Request $request)
    {
        try {
            ...
            if (! $wishlistItem) {
                return $this->addErrorResponse(422, trans('global.item_not_found_in_wishlist'), '');
            }

            $wishlistItem->delete();

            return $this->addSuccessResponse(200, trans('global.removed_from_wishlist_successfully'), '');
        } catch (\Exception $e) {
            return $this->addErrorResponse(500, trans('global.something_wrong'), $e->getMessage());
        }
    }
    ...
    public function getWishlist(Request $request)
    {
        ...
        return $this->addSuccessResponse(200, trans('global.wishlist_items_fetched_successfully'), ['items' => $wishlist]);
        try {
        } catch (\Exception $e) {
            return $this->addErrorResponse(500, trans('global.something_wrong'), $e->getMessage());
        }
